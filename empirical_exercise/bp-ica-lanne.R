...
init_exp <- find_init_cvm(res = u_chol,n_lhs = n_lhs,ordering = "lanne-saik",C = C)
...
Aidcvm <- maxfinder(A = init_exp$Acvm)$A.id
...
for(i in 1:n_lhs){
  c.ica[[i]] <- init_exp$A.ID.ica[[i]] %>% apply(MARGIN = 2, function(x) which.max(abs(x)))
  c.dc[[i]]  <- init_exp$A.ID.dc[[i]]  %>% apply(MARGIN = 2, function(x) which.max(abs(x)))
  c.pml[[i]] <- init_exp$A.ID.pml[[i]] %>% apply(MARGIN = 2, function(x) which.max(abs(x)))
}
...
negative_index <- find_negative(index.ica = index.ica,index.dc = index.dc,index.pml = index.pml,init_exp = init_exp)

unlist(lapply(negative_index$neg.ica, function(x) paste(x,collapse = "-"))) %>% as_tibble() %>% 
  group_by(value) %>% summarise(tot_type = n()) %>% 
  arrange(desc(tot_type))
unlist(lapply(negative_index$neg.pml, function(x) paste(x,collapse = "-"))) %>% as_tibble() %>% 
  group_by(value) %>% summarise(tot_type = n()) %>% 
  arrange(desc(tot_type))
unlist(lapply(negative_index$neg.dc, function(x) paste(x,collapse = "-"))) %>% as_tibble() %>%
  group_by(value) %>% summarise(tot_type = n()) %>% 
  arrange(desc(tot_type))
...
index.neg.ica <- do.call(rbind,lapply(negative_index$neg.ica, function(x) paste0(x,collapse = "-"))) %>% as.data.frame() %>% 
  mutate(neg_index = 1:n()) %>% 
  filter(str_detect(V1,"4-6")) %>% #slice(1) %>% 
  dplyr::select(neg_index) #%>% as.double()

index.neg.dc <- do.call(rbind,lapply(negative_index$neg.dc, function(x) paste0(x,collapse = "-"))) %>% as.data.frame() %>% 
  mutate(neg_index = 1:n()) %>% 
  filter(str_detect(V1,"4-6")) %>% #slice(1) %>% 
  dplyr::select(neg_index) #%>% as.double()

index.neg.pml <- do.call(rbind,lapply(negative_index$neg.pml, function(x) paste0(x,collapse = "-"))) %>% as.data.frame() %>% 
  mutate(neg_index = 1:n()) %>% 
  filter(str_detect(V1,"4-6")) %>% #slice(1) %>% 
  dplyr::select(neg_index) #%>% as.double()
...
Aidpml         <- init_exp$A.ID.pml[[final.index.pml]]
Aidica         <- init_exp$A.ID.ica[[final.index.ica]]
Aiddc          <- init_exp$A.ID.dc[[final.index.dc]]
...
save(point_irf,file = here("empirical_exercise","/lanne_point_irf_rep.RData"))
...
bootstrap.ica <- mb.boot.gigi(x = coef.ica,horizon = 20,nboot = boot_replications,
                              w00 = init_exp$w0[[final.index.ica]],method = "fastICA",set_seed = F,
                              ordering = "lanne-saik")


bootstrap.dc <- mb.boot.gigi(x = coef.dc,horizon = 20,nboot = boot_replications,
                             w00 = init_exp$w0[[final.index.dc]],method = "Distance covariances",set_seed = F,
                             ordering = "lanne-saik")

bootstrap.pml<- mb.boot.gigi(x = coef.pml,horizon = 20,nboot = boot_replications,
                             w00 = init_exp$w0[[final.index.pml]],method = "PML",set_seed = F,
                             ordering = "lanne-saik")
...
save(my_boot,file = here("empirical_exercise","lanne_bootstrap_irfs_ordering_frob_temp.RData"))
